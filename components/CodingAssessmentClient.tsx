...
export default function CodingAssessmentClient() {
  ...
  useEffect(() => {
    // Auto-enter fullscreen mode when questions load (optional, not mandatory)
    if (!isLoading && questions.length > 0 && !isFullscreen) {
      const timer = setTimeout(async () => {
        ...
        toast.info("Don't switch tabs! (3 strikes = auto-submit)");
      }, 300);
  ...
  useEffect(() => {
    // Monitor tab switches and auto-submit after 3
    const handleVisibilityChange = () => {
      if (document.hidden && isFullscreen && !hasSubmittedRef.current) {
        setTabSwitchCount((prev) => {
          const newCount = prev + 1;

          if (newCount >= 3) {
            toast.error("3 tab switches! Auto-submitting...");
            hasSubmittedRef.current = true;
            setTimeout(() => {
              handleSubmit();
            }, 500);
          } else {
            toast.warning(`Tab switch ${newCount}/3 detected!`);
          }
  ...
  const runCode = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("run-code API error:", response.status, errText);
  ...
  if (isSubmitting) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Loader size="lg" text="Evaluating your code and submitting..." />
      </div>
    );
  }
