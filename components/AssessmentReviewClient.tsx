...
const AssessmentReviewClient = memo(function AssessmentReviewClient() {
  ...
  const [explanations, setExplanations] = useState<
    Record<string, { text: string; references: string[] }>
  >({});
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log("Review data raw:", raw);
      if (!raw) {
        setIsLoading(false);
        return;
      }
      ...
      console.log("Total questions in review:", parsed.questions?.length);
      ...
      if (parsed.type === "coding") {
        ...
        console.log("Setting MCQ questions count:", allQuestions.length);
  ...
  return (
    <div className="flex flex-col gap-6 text-foreground">
      ...
      <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 mb-2">
        <h2 className="text-xl font-semibold mb-1">Assessment Review</h2>
        <p className="text-sm text-muted-foreground">
          Reviewing{" "}
          ...
          <div key={q.id} className="rounded-md border p-4">
            ...
            {explanations[q.id] && (
              <div className="mt-3 rounded-md bg-blue-50 border border-blue-200 p-4">
                ...
                {explanations[q.id].references.length > 0 && (
                  <div className="border-t border-blue-200 pt-3 mt-3">
                    <h5 className="text-xs font-semibold text-blue-900 mb-2">
                      References:
                    </h5>
                    <ul className="space-y-1">
                      {explanations[q.id].references.map((ref, i) => (
